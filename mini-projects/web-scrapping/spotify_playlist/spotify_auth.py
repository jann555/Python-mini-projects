...
def authenticate_spotify_user(spotify_user_creds):
    sp = spotipy.Spotify(
...
def get_song_list_uri(sp, song_names, date):
    song_uris = []
    year = date.split("-")[0]
...
def add_songs_to_spotify_playlist(spotify_user_creds, songs_list, date):
    sp = authenticate_spotify_user(spotify_user_creds)
    user_id = sp.current_user()["id"]
    song_uris = get_song_list_uri(sp=sp, song_names=songs_list, date=date)
